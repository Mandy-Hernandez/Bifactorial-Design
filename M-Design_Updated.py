...
global sheight, swidth, fh, fw, matrix_mod_rot_1, ma_f_1_rot_1, ma_f_2_rot_1, header_change_0, header_change_1, header_change_2, temp_input_data, coef_val_2, max_value, f_1, f_2, resp_f, \
    f_1max, f_2max, my_results, results_buffer_1, results_buffer, Results, r_data_input, Results_2,  select, n_replic, n_rows, designs_list, ma_f_1, matrix_f_1, matrix_f_2, rand_index, matrix_mod, matrix_f_1_1, matrix_f_2_1, ma_f_1_1, rand_index_1, matrix_mod_1, matrix_f_2_m, f_1_max, f_2_max, test, result, fact_1, fact_2, resp
# Very important, Define all the globals! :))
# Get the Display Resolution and calculate the conversion factor depending on the screen resolution
sheight = pyautogui.size()[1]
swidth = pyautogui.size()[0]
fh = round(sheight/1080)
fw = round(swidth/1920)
...
def processing():
    ...
    # Conditional Analysis, the plot will be done only when number of experimental points is higher than 8 or equal
    if len(fact_1) >= 8: # Check this part, try to fix the related bug
        # Extracting column data from Data Frame stored in buffer (temp_input_data)
...
class initial_window(QMainWindow):
    def __init__(self):
        super().__init__()
        self.gd = None
        self.setGeometry(500, 300, fw*720, fh*550)
        self.setFixedSize(fw*720, fh*550)
        self.setWindowTitle("M-Design: A project for Design of Experiments, ver 1.1 by Armando Hernández (2024)")
        self.insert_table = input_table(100, 3)
        self.setCentralWidget(self.insert_table)
        # Adding a Menu Bar with submenus
        my_bar = QToolBar()
        my_bar.setIconSize(QSize(int(30*fw), int(30*fh)))
        font_bar = QFont()
        font_bar.setPointSize(15)
        open_file = QAction(QIcon("./icons/blue-folder-horizontal-open1.png"), "Open File", self)  # Open file
        open_file.setFont(font_bar)
        open_file.setChecked(True)
        open_file.triggered.connect(self.insert_table.open_file)
        save_file = QAction(QIcon("./icons/disk-black.png"), "Save File as CSV", self)  # Save File
        save_file.setFont(font_bar)
        save_file.setChecked(True)
        save_file.triggered.connect(self.insert_table.save_data)
        define_factors = QAction(QIcon("./icons/factors_1.png"), "Define Factors", self)  # Define Factors
        ...
        gen_design = QAction(QIcon("./icons/generate_design.png"), "Generate Design", self)  # Generate Design
        gen_design.setFont(font_bar)
        gen_design.setChecked(True)
        gen_design.triggered.connect(self.gene_design)
        scatter_plot = QAction(QIcon("./icons/scatter-3d.png"), "Scatter Plot", self)  # Scatter Plot
        ...
        linear_fit = QAction(QIcon("./icons/linear3.png"), "Fit to Linear", self)  # Linear Fit
        ...
        quadratic_fit = QAction(QIcon("./icons/quadratic1.png"), "Fit to quadratic", self)  # Quadratic Fit
        ...
        plot_surface_1 = QAction(QIcon("./icons/surface1-plot.png"), "Plot Surface-1", self)  # Plot Surface-1
        ...
        plot_surface_2 = QAction(QIcon("./icons/surface2-plot.png"), "Plot Surface-2", self)  # Plot Surface-2
        ...
        present.setGeometry(fw*420, fh*60, fw*350, fh*20)
        font1=QFont()
        font1.setPointSize(12)
        present.setFont(font1)
        present.setText("BIFACTORIAL DESIGN ver. 1.1 (2024)")
        present_1 = QLabel(self)
        present_1.setGeometry(fw*400, fh*80, fw*350, fh*20)
        font2=QFont()
        font2.setPointSize(11)
        present_1.setFont(font2)
        present_1.setText("An open source project by Armando Hernández")
         # Logo
        logo_im = QLabel(self)
        logo_im.setGeometry(370*fw, 200*fh, 330*fw, 320*fh)
        logo_im.setFixedSize(fw*330,320*fh)
        logo_im.setAutoFillBackground(True)
        logo_im.setAlignment(Qt.AlignCenter)
        logo_im.setPixmap(QPixmap("./icons/test600.png"))
        # Help Option
...
class not_enough_data(QDialog):
    def __init__(self):
        super().__init__()
        self.setGeometry(600, 500, fw*450, fh*120)
        self.setFixedSize(fw*450, fh*120)
        font = QFont()
        font.setPointSize(12)
        font.setBold(True)
        self.setFont(font)
        n_e_data = QLabel(self)
        n_e_data.setGeometry(40, 20, fw*450, 50)
        n_e_data.setText("You are missing values, please verify your input!")
        ok_button = QPushButton(self)
        ok_button.setGeometry(int(210*fw), 80, 60, int(30*fh))
        ok_button.setText("OK")
        ok_button.clicked.connect(self.close)
        self.show()
# Need to complete the number of experiments
class not_enough_experiments(QDialog):
    def __init__(self):
        super().__init__()
        self.setGeometry(500, 300, fw*730, fh*120)
        self.setFixedSize(fw*730, fh*120)
        font = QFont()
        font.setPointSize(12)
        font.setBold(True)
        self.setFont(font)
        n_e_data = QLabel(self)
        n_e_data.setGeometry(20, 20, fw*730, 50*fh)
        n_e_data.setText('Complete the set of experiments or insert "0" values in the response to save the matrix')
        ok_button = QPushButton(self)
        ok_button.setGeometry(fw*350, 80*fh, 60*fw, 30*fh)
        ok_button.setText("OK")
        ok_button.clicked.connect(self.close)
        self.show()        
class wrong_data_input(QDialog):
    def __init__(self):
        super().__init__()
        self.setGeometry(500, 300, fw*500, fh*120)
        self.setFixedSize(fw*500, fh*120)
        font = QFont()
        font.setPointSize(12)
        font.setBold(True)
        self.setFont(font)
        n_e_data = QLabel(self)
        n_e_data.setGeometry(30, 20, fw*500, 50)
        n_e_data.setText("The VALUE is not a NUMBER,  please check your INPUT!")
        ok_button = QPushButton(self)
        ok_button.setGeometry(fw*210, 80, 60, fh*30)
        ok_button.setText("OK")
        ok_button.clicked.connect(self.close)
        self.show()
# Generate Design
class generate_design(QWidget):
    def __init__(self):
        ...
        self.setGeometry(800, 100, 300*fw, 280*fh)
        self.setFixedSize(300*fw, 280*fh)
        font = QFont()
        ...
        self.type_design.setGeometry(90, 50, 300*fw, 20*fh)
        self.type_design.setText("Choose your design")
        self.replicates.setFont(font)
        self.replicates.setGeometry(90, 120*fh, 300*fw, 40*fh)
        ...
        self.cont.setGeometry(120*fw, 230*fh, 100*fw, 30*fh)
...
class table_design_matrix(QTableWidget):
    def __init__(self):
        ...
        self.setGeometry(450, 200, 450*fw, 300*fh)
        self.setFixedSize(400*fw, 350*fh)
        self.setColumnWidth(0, 100*fw)
        self.setColumnWidth(1, 100*fw)
        self.setColumnWidth(2, 100*fw)
        self.init_ui()
    ...
    # Def Current Cell
    def c_current(self):
        global temp_input_data, result, fact_1, fact_2, resp, test, n_rows
        result = []
    ...
    # For now insert the functions to calculate. Otherwise, create calculation modules to import for using in every stage
    #  Def calculations
    def data_in_table_for_plot_scattered(self):
        self.c_current()
        if len(resp) < n_rows:
            self.not_enough_experiments_connection()
            return
        else:
    ...
    def factors_names_connection(self):
        ...
        self.c_current()
        return
        # Window to pop up "Wrong Input, not a Number!"
    def wrong_input_connection(self):
        global temp_input_data
        if self.ne is None:
            self.ne = wrong_data_input()
            self.ne.show()
        elif wrong_data_input():
            self.ne = wrong_data_input()
            self.ne.show()
        else:
            self.ne.close()
            self.ne = None
    # Window to pop up "Not enough experiments"
    def not_enough_experiments_connection(self):
        global temp_input_data
        if self.ne is None:
            self.ne = not_enough_experiments()
            self.ne.show()
        elif not_enough_experiments():
            self.ne = not_enough_experiments()
            self.ne.show()
        else:
            self.ne.close()
...
# Saving Data as CSV # 
    def save_data(self):
        ...
        self.c_current()
        if len(resp) < 8:
            self.not_enough_experiments_connection() # Change this and adapt the script below to save the matrix without data input
            return
        else:
...
class design_matrix(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setGeometry(450, 200, 400*fw, 300*fh)
        self.setFixedSize(400*fw, 380*fh)
        ...
        my_bar = QToolBar()
        my_bar.setIconSize(QSize(30*fw, 30*fh))
        font_bar = QFont()
        font_bar.setPointSize(15)
        save_file = QAction(QIcon("./icons/disk-black.png"), "Save File as CSV", self)  # Save File
        save_file.setFont(font_bar)
...
class plot_scattered_canvas(FigureCanvas):
    ...
    def __init__(self, parent):
        ...
        fact_names()
        plt.close(1)
        self.sc_plot = plt.figure(1, figsize=(fw*5, fh*4))
        super().__init__(self.sc_plot)
        self.setParent(parent)
        self.eje1 = plt.axes(projection="3d")
        self. eje1.scatter(f_1, f_2, resp_f, s=fw*70, c="purple")
        self.eje1.set_title("Scatter Plot of Experimental Points").set_fontsize(20)
        self.eje1.set_xlabel(f_1_name, labelpad=20).set_fontsize(20)
        self.eje1.set_ylabel(f_2_name, labelpad=20).set_fontsize(20)
        self.eje1.set_zlabel(resp_name, labelpad=20).set_fontsize(20)
        self.eje1.tick_params(labelsize=20, direction='in', labelfontfamily='arial')
        plt.draw()
        return
...
class plot_scattered(QMainWindow):
    def __init__(self):
        super().__init__()
        self.close()
        self.setGeometry(100*fw, 100*fh, fw*500, fh*400)
        self.setWindowTitle("Scatter Plot of Experimental Points")
        self.setFixedSize(fw*700, fh*600)
        self.plot_surface = plot_scattered_canvas(self)
        self.setCentralWidget(self.plot_surface)
        bar = QToolBar()
        bar.setIconSize(QSize(fw*30, fh*30))
        bar.setFixedSize(fw*500, fh*30)
        self.addToolBar(bar)
        save_fig = QAction(QIcon("./icons/disk-black.png"), "Save Figure", self)
...
class plot_first_order_canvas(FigureCanvas):
    ...
    def __init__(self, parent):
        ...
        self.sc_plot = plt.figure(2, figsize=(fw*5, fh*4))
        super().__init__(self.sc_plot)
        self.setParent(parent)
        self.eje1 = plt.axes(projection="3d")
        self. eje1.scatter(f_1, f_2, resp_f, s=fw*70, c="purple")
        self.eje1.set_title("Response Surface, Linear Model with Interactions").set_fontsize(20)
        self.eje1.set_xlabel(f_1_name, labelpad=20).set_fontsize(20)
        self.eje1.set_ylabel(f_2_name, labelpad=20).set_fontsize(20)
        self.eje1.set_zlabel(resp_name, labelpad=20).set_fontsize(20)
        self.eje1.tick_params(labelsize=20, direction='in', labelfontfamily='arial')
...
class plot_first_order(QMainWindow):
    def __init__(self):
        super().__init__()
        self.close()
        self.setGeometry(int(400*fw), int(100*fh), 500, 400)
        self.setWindowTitle("Response Surface, Linear Model with Interactions")
        self.setFixedSize(fw*700, fh*600)
        self.plot_surface1 = plot_first_order_canvas(self)
        self.setCentralWidget(self.plot_surface1)
...
class plot_first_order_projection_canvas(FigureCanvas):
    ...
    def __init__(self, parent):
        ...
        fact_names()
        plt.close(3)
        my_results = pd.read_csv(io.StringIO(results_buffer_1.getvalue())) #check a better syntaxis for this line
        const = my_results[" Coefficient  "].explode()
        s1 = (max(f_1) - min(f_1)) / 100
        s2 = (max(f_2) - min(f_2)) / 100
        x = np.arange(min(f_1), max(f_1), s1)
        ...
        self.sc_plot = plt.figure(3, figsize=(fw*5, fh*(4)))
        self.eje1 = plt.axes()
        self.eje1.set_title("Surface Projection, Linear Model with Interactions").set_fontsize(20)
        self.eje1.set_xlabel(f_1_name, labelpad=20).set_fontsize(20)
        self.eje1.set_ylabel(f_2_name, labelpad=20).set_fontsize(20)
        self.eje1.tick_params(labelsize=20, direction='in', labelfontfamily='arial')
...
class plot_first_order_projection(QMainWindow):
    def __init__(self):
        super().__init__()
        self.close()
        self.setGeometry(fw*400, fh*500, 500, 400)
        self.setWindowTitle("Surface Projection, Linear Model with Interactions")
        self.setFixedSize(fw*700, fh*600)
        self.plot_proj1 = plot_first_order_projection_canvas(self)
        self.setCentralWidget(self.plot_proj1)
        bar = QToolBar()
        bar.setIconSize(QSize(fw*30, fh*30))
        bar.setFixedSize(fw*500, fh*30)
        self.addToolBar(bar)
...
class plot_quadratic_canvas(FigureCanvas):
    ...
    def __init__(self, parent):
        ...
        const = my_results[" Coefficient  "].explode()
        self.sc_plot = plt.figure(4, figsize=(fw*5, fh*(4)))
        self.eje1 = plt.axes(projection="3d")
        self. eje1.scatter(f_1, f_2, resp_f, s=fw*70, c="purple")
        self.eje1.set_title("Response Surface, Quadratic Model with Interactions").set_fontsize(20)
        self.eje1.set_xlabel(f_1_name, labelpad=20).set_fontsize(20)
        self.eje1.set_ylabel(f_2_name, labelpad=20).set_fontsize(20)
        self.eje1.set_zlabel(resp_name, labelpad=20).set_fontsize(20)
        self.eje1.tick_params(labelsize=20, direction='in', labelfontfamily='arial')
...
class plot_quadratic(QMainWindow):
    def __init__(self):
        super().__init__()
        self.close()
        self.setGeometry(fw*300, fh*100, 500, 400)
        self.setWindowTitle("Response Surface, Quadratic Model with Interactions")
        self.setFixedSize(fw*700, fh*600)
        self.plot_surface2 = plot_quadratic_canvas(self)
        self.setCentralWidget(self.plot_surface2)
        bar = QToolBar()
        bar.setIconSize(QSize(QSize(fw*30, fh*30)))
        bar.setFixedSize(fw*500, fh*30)
        self.addToolBar(bar)
        save_fig = QAction(QIcon("./icons/disk-black.png"), "Save Figure", self)
        save_fig.setCheckable(True)
...
class plot_quadratic_projection_canvas(FigureCanvas):
    ...
    def __init__(self, parent):
        ...
        for i in range(0, len(z)):
            planes.append(float(d[i, 2]))
        planes = sorted(planes)
        self.sc_plot = plt.figure(5, figsize=(fw*5, fh*(4)))
        self.eje1 = plt.axes()
        self.eje1.set_title("Surface Projection, Quadratic Model with Interactions").set_fontsize(20)
        self.eje1.set_xlabel(f_1_name, labelpad=20).set_fontsize(20)
        self.eje1.set_ylabel(f_2_name, labelpad=20).set_fontsize(20)
        self.eje1.tick_params(labelsize=20, direction='in', labelfontfamily='arial')
        plt.contour(X, Y, Z, levels=planes[0:-1], cmap="winter")
        plt.contour(X, Y, Z, colors="black")
        self.eje1.scatter(f_1max, f_2max, s=fw*70, c="red")
        self.eje1.annotate("Optimal", xy=(f_1max+0.01, f_2max+0.01), c="red").set_fontsize(20)
        plt.draw()
        super().__init__(self.sc_plot)
        self.setParent(parent)
# Second Order Projection Plot Main
class plot_quadratic_projection(QMainWindow):
    def __init__(self):
        super().__init__()
        self.close()
        self.setGeometry(fw*300, fw*500, 500, 400)
        self.setWindowTitle("Surface Projection, Quadratic Model with Interactions")
        self.setFixedSize(fw*700, fh*600)
        self.plot_proj2 = plot_quadratic_projection_canvas(self)
        self.setCentralWidget(self.plot_proj2)
        bar = QToolBar()
        bar.setIconSize(QSize(fw*30, fh*30))
        bar.setFixedSize(fw*500, fh*30)
        self.addToolBar(bar)
...
class first_order_fitting(QTableWidget):
    def __init__(self):
        ...
        save_data.setIcon(QIcon("./icons/disk-black.png"))
        save_data.setIconSize(QSize(fw*25, fh*25))
        ...
        self.setGeometry(800, 100, fw*1000, fh*300)
        self.setFixedSize(fw*1000, fh*300)
        self.setColumnCount(5)
        self.setColumnWidth(0, fw*110)
        self.setColumnWidth(1, fw*110)
        self.setColumnWidth(2, fw*150)
        self.setColumnWidth(3, fw*300)
        self.setColumnWidth(4, fw*300)
        self.setRowCount(5)
        ...
        self.r_squared.setGeometry(fw*200, fh*150, fw*210, fh*100)
        self.r_squared.setText("R-squared = " + str(round(det_coeff, 4)))
        self.f_value.setGeometry(fw*200, fh*180, fw*210, fw*100)
        self.f_value.setText("F-value = " + str(round(Fisher_Val, 4)))
        self.prob_f_value.setGeometry(fw*200, fh*210, fw*220, fw*120)
...
class second_order_fitting(QTableWidget):
    def __init__(self):
        ...
        self.setGeometry(800, 500, fw*1000, fh*300)
        self.setFixedSize(fw*1000, fh*400)
        self.setColumnCount(5)
        self.setColumnWidth(0, fw*110)
        self.setColumnWidth(1, fw*110)
        self.setColumnWidth(2, fw*150)
        ...
        self.r_squared.setGeometry(fw*250, fh*240, fw*210, fh*100)
        self.r_squared.setText("R-squared = " + str(round(det_coeff, 4)))
        self.f_value.setGeometry(fw*250, fh*270, fw*210, fw*100)
        self.f_value.setText("F-value = " + str(round(Fisher_Val, 4)))
        self.prob_f_value.setGeometry(fw*250, fh*300, fw*210, fw*100)
        self.prob_f_value.setText("Probability(F) = " + str(round(Fisher_prob, 4)))
        self.max_resp.setGeometry(fw*620, fh*250, fw*220, fh*100)
        self.max_resp.setText("Maximum Value = " + str(round(max_value, 2)))
        self.f1_max.setGeometry(fw*620, fh*280, fw*220, fh*100)
        self.f1_max.setText(f_1_name + " = " + str(round(f_1max, 2)))
        self.f2_max.setGeometry(fw*620, fh*310, fw*220, fh*100)
        ...
        for u in range(0, len(t_prob_item_2)):
            # t_probability
            tp_item_2.append(QTableWidgetItem(str(round(t_prob_item_2[u], 4))))
            # t-value
...
class input_table(QTableWidget):
    global temp_input_data, f_1_name, f_2_name, resp_name, header_change_0, header_change_1, header_change_2, result, fact_1, test, fact_2, resp
    def __init__(self, r, c):
        ...
        font.setPointSize(12)
        self.setFont(font)
        self.setGeometry(800, 100, fw*720, fh*500)
        self.setFixedSize(fw*370, fh*500)
    ...
    # Def Current Cell
    def c_current(self):
        global temp_input_data, result, fact_1, test, fact_2, resp
        result = []
    ...
    def check_values(self):
        global temp_input_data, result, fact_1, test, fact_2, resp
        # Checking input value fact_1 not number
        for i in fact_1:
            check_fact_1 = re.findall("[^0-9.-]", i)
            if check_fact_1:
                self.wrong_input_connection()
                return
        for j in fact_2:
            check_fact_2 = re.findall("[^0-9.-]", j)
            if check_fact_2:
                self.wrong_input_connection()
                return
        for k in resp:
            check_fact_3 = re.findall("[^0-9.-]", k)
            if check_fact_3:
                self.wrong_input_connection()
                return
        # Array of values for further calculations
        test = np.array([fact_1, fact_2, resp], dtype=object)
        test = np.transpose(test)
        test_data = pd.DataFrame(test)
        # Saving to buffer
    ...
    def data_in_table_for_plot_scattered(self):
        self.c_current()
        if len(fact_1) < 8 or len(fact_2) < 8 or len(resp) < 8:
            self.not_enough_data_connection()
            return
        elif len(fact_1) >= 8 and len(fact_1) == len(resp) and len(fact_1) == len(fact_2):
            self.plot_scattered_connection()
            return
