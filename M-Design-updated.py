...
class initial_window(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("M-Design: A project for Design of Experiments, ver 1.1, by Armando Hernandez (2024)")
        self.insert_table = input_table(100, 3)
        self.setCentralWidget(self.insert_table)
        # Adding a Menu Bar with submenus
        my_bar = QToolBar()
        my_bar.setIconSize(QSize(30, 30))
        font_bar = QFont()
        font_bar.setPointSize(15)
        open_file = QAction(QIcon("blue-folder-horizontal-open.png"), "Open File", self)  # Open file
        open_file.setFont(font_bar)
        open_file.setChecked(True)
        open_file.triggered.connect(self.insert_table.open_file)
        save_file = QAction(QIcon("disk-black.png"), "Save File as CSV", self)  # Save File
        save_file.setFont(font_bar)
        save_file.setChecked(True)
        save_file.triggered.connect(self.insert_table.save_data)
        define_factors = QAction(QIcon("factors_1.png"), "Define Factors", self)  # Define Factors
        ...
        gen_design = QAction(QIcon("generate_design.png"), "Generate Design", self)  # Generate Design
        gen_design.setFont(font_bar)
        gen_design.setChecked(True)
        gen_design.triggered.connect(self.gene_design)
        scatter_plot = QAction(QIcon("scatter-3d.png"), "Scatter Plot", self)  # Scatter Plot
        ...
        linear_fit = QAction(QIcon("linear3.png"), "Fit to Linear", self)  # Linear Fit
        ...
        quadratic_fit = QAction(QIcon("quadratic1.png"), "Fit to quadratic", self)  # Quadratic Fit
        ...
        plot_surface_1 = QAction(QIcon("surface1-plot.png"), "Plot Surface-1", self)  # Plot Surface-1
        ...
        plot_surface_2 = QAction(QIcon("surface2-plot.png"), "Plot Surface-2", self)  # Plot Surface-2
...
class not_enough_data(QDialog):
    def __init__(self):
        ...
        n_e_data.setGeometry(40, 20, 450, 50)
        n_e_data.setText("At LEAST 8 EXPERIMENTAL POINTS are needed!")
        ok_button = QPushButton(self)
        ok_button.setGeometry(210, 70, 60, 30)
...
class not_enough_experiments(QDialog):
    def __init__(self):
        super().__init__()
        self.setGeometry(500, 300, 570, 180)
        ...
        self.setFont(font)
        n_e_data = QLabel(self)
        n_e_data.setGeometry(20, 20, 540, 50)
...
class wrong_data_input(QDialog):
    def __init__(self):
        ...
        self.setFont(font)
        n_e_data = QLabel(self)
        n_e_data.setGeometry(30, 20, 400, 30)
        n_e_data.setText("The VALUE is not a NUMBER,  please check your INPUT!")
        ok_button = QPushButton(self)
        ok_button.setGeometry(200, 60, 60, 30)
        ok_button.setText("OK")
        ok_button.clicked.connect(self.close)
        self.show()
# Generate Design
...
class plot_scattered_canvas(FigureCanvas):
    ...
    def __init__(self, parent):
        ...
        self. eje1.scatter(f_1, f_2, resp_f, s=50, c="purple")
...
class plot_scattered(QMainWindow):
    def __init__(self):
        ...
        self.addToolBar(bar)
        save_fig = QAction(QIcon("disk-black.png"), "Save Figure", self)
...
class plot_quadratic_canvas(FigureCanvas):
    ...
    def __init__(self, parent):
        ...
        const = my_results[" Coefficient  "].explode()
        self.sc_plot = plt.figure(4, figsize=(6, 4))
        self.eje1 = plt.axes(projection="3d")
        self. eje1.scatter(f_1, f_2, resp_f, s=50, c="purple")
...
class plot_quadratic_projection_canvas(FigureCanvas):
    ...
    def __init__(self, parent):
        ...
        for i in range(0, len(z)):
            planes.append(float(d[i, 2]))
        planes = sorted(planes)
        self.sc_plot = plt.figure(5, figsize=(6, 4))
        self.eje1 = plt.axes()
        ...
        plt.contour(X, Y, Z, levels=planes[0:-1], cmap="winter")
        plt.contour(X, Y, Z, colors="black")
        self.eje1.scatter(f_1max, f_2max, s=70, c="red")
        self.eje1.annotate("Optimal", xy=(f_1max+0.01, f_2max+0.01), c="red")
        plt.draw()
        super().__init__(self.sc_plot)
        self.setParent(parent)
# First Order Projection Plot Main
...
class first_order_fitting(QTableWidget):
    def __init__(self):
        ...
        save_data.setIcon(QIcon("disk-black.png"))
        ...
        self.prob_f_value.setGeometry(180, 210, 220, 100)
...
class second_order_fitting(QTableWidget):
    def __init__(self):
        ...
        self.f_value.setText("F-value = " + str(round(Fisher_Val, 4)))
        self.prob_f_value.setGeometry(220, 310, 220, 100)
        self.prob_f_value.setText("Probability(F) = " + str(round(Fisher_prob, 10)))
...
class input_table(QTableWidget):
    ...
    def __init__(self, r, c):
        ...
        logo_im.setPixmap(QPixmap("presentation.png"))
    # Def Init_UI
    def init_ui(self):
        self.cellChanged.connect(self.c_current)
        self.cellEntered.connect(self.c_current)
